...
(function(html) {
    ...
    const cfg = {
        ...
        finalDate : 'December 22, 2023 00:00:00',
        // MailChimp URL
    ...
    * ---------------------------------------------------- */ 
    const ssMailChimpForm = function() {
        ...
        function hasError(field) {
            ...
            if (validity.valueMissing) return 'Please enter all inputs.';

            // If not the right type
            if (validity.typeMismatch) {
                if (field.type === 'email') {return 'Please enter a valid email address.';}
                else if (field.type === 'tel') {return 'Please enter a valid phone number.';}
                else if (field.type === 'text') {return 'Lol! Please enter a valid name.';}
            }
        ...
        // Submit the form 
        function submitMailChimpForm(mcForm) {
            ...
            let formData = new FormData(mcForm); // creaing new object from form data
            xhr.send(formData); //sendung data to php
        ...
        mcForm.addEventListener('submit', function (event) {
            ...
            let numberField = event.target.querySelector('#mce-NUMBER');
            let nameField = event.target.querySelector('#mce-NAME');
            let error = hasError(numberField, nameField, emailField);
            let error3 = hasError(nameField);
            let error2 = hasError(emailField);

            if (error2) {
                showError(emailField, error2);
                emailField.focus();
                return;
            }else if (error3) {
                showError(nameField, error3);
                nameField.focus();
                return;
            }else if (error) {
                showError(numberField, error);
                numberField.focus();
                return;
            }
